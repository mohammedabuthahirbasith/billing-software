...
## 2026-07-26 — Visual Redesign: Sidebar Shell, Real Dashboard, Illustrated Auth Pages

**Phase:** Frontend — App Shell & First Impressions
**Files:** `components/Layout.jsx` (rewrite), `pages/Dashboard.jsx` (rewrite), `components/AuthLayout.jsx` (new), `pages/Login.jsx`, `pages/Register.jsx`, `index.css` (theme tokens)

### What it does

- **`Layout.jsx`** — replaces the old centered top-nav shell (`mx-auto max-w-6xl` on both header and content, leaving large idle margins on anything wider than a laptop) with a persistent, fixed, dark sidebar on desktop (`lg:` and up) and a slim top bar + slide-in drawer below it. The drawer closes on link click, backdrop click, or Escape — the same interaction contract `ConfirmDialog` already established elsewhere in the app. Content area widens from `max-w-6xl` to `max-w-screen-2xl`, offset by `lg:pl-64` to sit beside the sidebar.
- **`Dashboard.jsx`** — replaces the old two-link welcome hub with an actual data dashboard: OWNER gets today's revenue/GST/invoice-count stat tiles (reusing the existing `/api/reports/sales?from=<today>&to=<today>` endpoint, no backend changes); every role gets a 5-item recent-invoices list (`/api/invoices`, clickable rows); quick-action tiles are gated to match each action's real `@PreAuthorize` rule exactly — "New Invoice" for everyone, "Add Product"/"View Reports"/"Add Staff" for OWNER only.
- **`AuthLayout`** — a full-page background image for Login/Register, with the form floating on top as a centered card. First pass (no real image available yet) split the page into two columns and filled the left half with an SVG/CSS illustration; user feedback was that this read as messy and disjointed, like two unrelated pages stitched together. Second pass unified that into one full-bleed CSS illustration behind a centered card — better, but still not a real image. The user then supplied an actual illustrated background image, which is what's shipped now: the 4K source was resized to 1920px wide and re-encoded as JPEG (`public/auth-background.jpg`, ~42KB — down from ~9MB) so it never becomes a load-time cost, set as `background-size: cover` behind a soft dark overlay (`bg-navy-950/40`) so the form stays legible regardless of which part of the image lands behind it. The existing `Card`-wrapped form sits centered on top at every viewport width (mobile included), with a heavier `shadow-2xl` (up from the default `shadow-sm`) so it reads as a floating panel against the busier background rather than the flat page it was designed for originally. (A `brand-600` accent strip along the card's top edge was tried first, but with a visible gap before the card's rounded corners it read as a stray line rather than a deliberate accent — removed rather than "fixed in place," since a plain card was the simpler and cleaner result anyway.) `Login.jsx`/`Register.jsx` had their centered-div wrapper swapped for `<AuthLayout>` with zero changes to form state, handlers, or validation.
- **Color theme retrofit (`index.css`)** — after seeing the first color pass (a generic indigo accent + plain `slate-900` sidebar), the user flagged that neither the sidebar nor the login card felt like they belonged to a *billing* product specifically. Rather than guess at a new palette, this was researched against how established billing/invoicing and fintech products actually theme themselves (Vyapar, myBillBook, Zoho Books, QuickBooks Online, Razorpay, FreshBooks) before choosing a direction, then confirmed with the user as one of three concrete, differently-sourced options. The chosen direction mirrors Razorpay's Blade design system: a deep prussian-navy chrome (new `--color-navy-900: #0B2545` for the sidebar, `--color-navy-800`/`-700` for hover/border states, `--color-navy-950` for overlays) paired with a brighter blue accent (new `--color-brand-*` scale centered on `#1668E3`, replacing the earlier indigo alias) reserved for active nav items, links, and primary buttons. Because every button/link in the app already referenced the `brand-*` alias rather than raw `indigo-*` classes, repointing that one `@theme` block in `index.css` repainted the whole app's accent color with no per-component edits.

### Why it's written this way

- **Sidebar over top-nav is the standard shape for a serious admin product**, not just a wider version of the old bar — the dark/light contrast plus a pinned nav column is what makes the shell read as "built for daily use" rather than "marketing site with a login." This was the direct fix for "left and right side having spaces."
- **Content capped at `max-w-screen-2xl`, not uncapped.** An unbounded width looks broken on a 4K/ultrawide monitor — very wide tables with huge gaps between sparse columns read as a bug, not spaciousness. 1536px is wide enough that virtually no common laptop/desktop viewport feels constrained, while still capping the extreme case.
- **Forms deliberately stay at their existing constrained widths** (`InvoiceForm`, `ProductForm`, `StaffForm` all keep `max-w-2xl`/`max-w-md`). Wide free-form inputs are harder to scan — Stripe, Linear, and every other serious dashboard product constrain form width even inside a full-width shell. Only the shell itself and data-dense pages (tables, Dashboard, Reports) get the extra room; this is a considered choice, not an inconsistency.
- **Dashboard reuses an existing endpoint instead of adding a new one.** `ReportService`'s sales-summary aggregate already supports an arbitrary date range; asking for `from=today&to=today` gets "today's numbers" for free with no new backend surface area.
- **Quick actions mirror `@PreAuthorize` exactly, not "whatever looks nice."** A CASHIER never sees a shortcut into something they'd get a 403 on — the Dashboard doesn't become an accidental backdoor around the role boundaries `ReportController`/`ProductController`/`AuthController` already enforce.
- **One unified background beats a segregated split-screen.** The first draft's 50/50 layout technically delivered "an image on the login page," but a hard seam down the middle — vivid illustration on one side, flat white panel on the other — read as two disconnected halves rather than one designed page. A full-bleed background with the form floating on top as a card reads as one thing, not two — and that composition carried over unchanged once the CSS illustration was swapped for the real image.
- **Resizing/re-encoding a supplied asset before shipping it isn't optional polish.** A 4K PNG straight off disk was ~9MB — landing that on a login page (the very first thing a mobile user waits on, already a known pain point after the Render cold-start investigation) would undo every other loading-state improvement made this project. Resizing to the actual rendered width and re-encoding as JPEG got it under 50KB with no visible quality loss at typical viewport sizes.
- **`public/` over an imported asset.** Vite's `src/assets` + `import` path is right for small assets that benefit from content-hashed cache-busting bundled into the JS graph; a single large background image referenced by a stable CSS `url()` is simpler served untouched from `public/` and requires no changes to how it's referenced if it's ever swapped again.
- **A distinct chrome color from the accent color, not one hue doing both jobs.** Razorpay's own design system deliberately separates a dark "chrome" tone (used for large surfaces — sidebars, backgrounds) from a brighter "accent" tone (used only for small, interactive surfaces — active states, links, primary buttons). Reusing the bright accent as the sidebar's background color would either wash it out at large area (if kept bright) or dull it everywhere once darkened to be usable at that size (if not). Two coordinated scales — `navy-*` for chrome, `brand-*` for accent — avoids that trade-off entirely.
- **Green intentionally stays out of the theme palette.** Every product researched (most explicitly QuickBooks: "don't use QuickBooks green outside the logo") treats green as a reserved *status* signal — paid, completed, GST-filed — not a brand color. This app's `Badge` component already only uses green (`emerald`) for a `success` tone, never for navigation or primary actions, so no change was needed there — the research confirmed an existing choice rather than requiring a new one.

### Interview talking points

- **Progressive disclosure by viewport, not by breakpoint hacks** — the mobile drawer and the desktop sidebar are two different DOM subtrees gated by Tailwind's `lg:` responsive prefix, each optimized for its own interaction model (tap-to-open drawer vs. always-visible rail), rather than one layout awkwardly squeezed to fit both.
- **Consistent interaction contracts across components** — the sidebar drawer's dismiss behavior (Escape/backdrop/action-click) deliberately matches `ConfirmDialog`'s, so a user only has to learn "how do I close this" once for the whole app.
- **Least-privilege UI, not just least-privilege API** — showing quick actions that match `@PreAuthorize` exactly is defense in depth applied to UX: the backend was already secure, but a UI that dangles an action a user can't actually perform erodes trust in the product even when it's not a security bug.
- **Honest constraints over fake polish** — with no image-generation or stock-photo tool available, the interim CSS illustration was built from primitives already proven out elsewhere in the app (gradients, blur, the barcode pattern) and the limitation was stated directly rather than glossed over; once the user supplied a real image, the fix was to properly resize/compress and ship it, not to keep the placeholder out of inertia.
- **Ground subjective design feedback in comparable products, not personal taste** — "doesn't feel like billing software" is a hard note to act on directly; translating it into "what color/layout language do Vyapar, Zoho Books, QuickBooks, and Razorpay actually use, and why" turns a vague aesthetic complaint into a small set of concrete, defensible options the user can choose between.
- **One token change, whole-app effect** — because the accent color was already centralized behind a `brand-*` alias (set up earlier for exactly this reason) rather than sprinkled as raw Tailwind color classes, the entire theme repaint was a single `@theme` block edit, not a find-and-replace across a dozen components.